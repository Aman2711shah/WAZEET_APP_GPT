...
router.post('/application', applicationLimiter, apiKeyAuth, // optional: enable API key auth if API_KEY env is set
uploadMiddleware, async (req, res) => {
    try {
        ...
        const { firestoreDocId, documents, applicationId } = await saveServiceApplicationToFirebase(payload, filesWithTypes);
